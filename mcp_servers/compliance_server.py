"""
FastMCP server: isolated compliance & policy enforcement.

Deliberately run as a *separate* MCP server/process from github_server.py so
the "auditor" has no GitHub write access at all -- it can only read code
handed to it and return findings. This mirrors a real separation-of-duties
setup where the remediation agent cannot also be its own sole reviewer.
"""
